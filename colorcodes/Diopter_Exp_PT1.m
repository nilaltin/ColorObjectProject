...
backlum = 0.3; % background luminance of the screen
textSize = 40;
...
nImages = size(T, 1);
diopter_List = [0 1 2 3 4 5 6 7 8]; % diopter blurring to be included % DEBUGGING
...
for t = 1:length(trialOrder) % 1:4 DEBUGGING
    ...
    if TS(trl).objstyle == 0 % non-Adobe
        if TS(trl).color == 0 && TS(trl).object == 0 % object absent / grayscale
            ...
            % diopter multiplied by 10 so don't have decimal points in the
            % filename
